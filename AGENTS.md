...
- `quic.rs`（适配层，M8.5）：`quic::Algorithm` 的密钥材料入口是
  ...
  曾以 "rx keys" panic 拦截）；
- `sign.rs`（Ed25519 点运算常数，2026-09-17 性能修复）：`curve_d()`
  （返回 `2d` 的 `&'static` 缓存）与 `base_point()` 均为 `OnceLock`
  缓存——旧形态每次 `add()` 现场派生 `2d`（Fermat 模逆 ~500 域乘 ×
  每标量乘 ~1024 次加法）导致 sign/verify 26 ms；改回"每次计算"
  即回归 120 倍。d 与 G 是公开曲线常数，缓存无零化/ct 顾虑；缓存
  值仍由同一派生式计算（向量测试零改动守护等价性）；
- `sha2.rs`（SHA-256 消息调度，2026-09-17 三轮实测定稿）：**软流水
  形态为当前最优**——64 字数组 + `w[i+16]` 融合进主循环第 i 轮头部
  （`if i < 48` 在完全展开后是编译期常量），较独立展开循环的两段
  式默认快 ~14%（445 MB/s，BENCHMARKS §5.6）。三条已实测否决的
  改写勿重复：16 字滚动窗口融合（默认/+avx2 慢 ~3–5%，§5.5）、
  16 字窗口分块调度（比软流水慢 ~9%，§5.6）、`std::simd` u32x4
  组调度（默认无增益、+avx512 回归 ~7%，§5.7——压缩轮串行链无
  lane 并行可开发）；宽 ISA 的 LLVM 决策漂移由 +75% 收窄至
  +65%，属已知容忍项；
