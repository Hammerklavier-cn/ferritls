...
//!
//! 外部锚定：AES 层已由 FIPS-197/GCM KAT 验证；本模块以往返 + 篡改 +
//! 跨块长度自洽测试覆盖（CAVP CCMVS 向量子集在 M7 引入——见 tests 注记）。
//! 上电自检覆盖（M5）。
    ...
    ($name:ident, $nonce_len:expr, $doc:expr) => {
        ...
        impl $name {
            ...
            fn cbc_mac(&self, nonce: &[u8; $nonce_len], aad: &[u8], pt: &[u8]) -> [u8; 16] {
                ...
                // B0: flags || nonce || 明文长度（L 字节 BE）
                buf.push(Self::B0_FLAGS);
                buf.extend_from_slice(nonce);
                // 明文长度用 L 字节 BE；此处明文 < 2^(8L) 由调用方协议保证，
                // 超长截断在 CCM 语义内本就非法——按位取低 L 字节
                let plen = pt.len();
            ...
            fn ctr_block(&self, nonce: &[u8; $nonce_len], counter: u16) -> [u8; 16] {
                let mut block = [0u8; 16];
                block[0] = Self::CTR_FLAGS;
                block[1..1 + $nonce_len].copy_from_slice(nonce);
                // 计数器占 L 字节（u16 足够覆盖常规记录，高位自然为 0）
                let lo = Self::LEN_BYTES.min(2);
                for i in 0..lo {
                    block[16 - Self::LEN_BYTES + i] = (counter >> (8 * (lo - 1 - i))) as u8;
                }
                self.aes.encrypt_block(&mut block);
                block
            }

            fn ctr_xor(&self, nonce: &[u8; $nonce_len], start: u16, data: &[u8]) -> Vec<u8> {
...
mod tests {
    ...
    fn round_trip_and_tamper() {
        ...
        for len in [0usize, 1, 15, 16, 17, 33, 64] {
            let pt: Vec<u8> = (0..len).map(|i| i as u8).collect();
            let sealed = tls.seal(&nonce12, b"aad", &pt);
            assert_eq!(sealed.len(), len + 16);
        ...
        assert_ne!(
            aead.seal(&nonce13, b"aad", b"x"),
            tls.seal(&nonce12, b"aad", b"x")
        );
    }
}
