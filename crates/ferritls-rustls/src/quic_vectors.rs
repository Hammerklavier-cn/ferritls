//! RFC 9001（QUIC-TLS）附录 A 样本保护向量。
//!
//! 来源：<https://www.rfc-editor.org/rfc/rfc9001>（2026-09 下载自
//! rfc-editor 原文，程序化提取；提取脚本对每个字段做了派生关系
//! 校验：nonce = iv⊕pn、掩码位应用、sample 取位，全部闭合）。
//! 人工录入禁改；如需再生成，见 docs/VECTOR-PROVENANCE.md。
...
pub(crate) const A2_PAYLOAD_PLAIN: &[u8] = &[
    ...
    0x04, 0x80, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
/// §A.2 最终受保护包（1200 字节）。
pub(crate) const A2_PROTECTED_PACKET: &[u8] = &[
...
/// §A.3 服务端 Initial 未保护载荷（99 字节：ACK + CRYPTO 帧）。
pub(crate) const A3_PAYLOAD_PLAIN: &[u8] = &[
...
/// §A.3 最终受保护包（135 字节）。
pub(crate) const A3_PROTECTED_PACKET: &[u8] = &[
