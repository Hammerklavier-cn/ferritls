...
//! 在此扩展即可。
//!
//! `public_key_alg_id()` / `signature_alg_id()` 返回**完整 DER 编码的
//! AlgorithmIdentifier**（pki-types 的 `(&'static [u8])` 形态），
//! webpki 与证书逐字节比对：
//! - EC：公钥 = ecPublicKey + 命名曲线 OID；签名 = ecdsa-with-SHA*
//!   （RFC 5758：不带 NULL 参数）；
//! - Ed25519：公钥/签名同 OID（1.3.101.112，无参数）；
//! - RSA PKCS#1：公钥 = rsaEncryption + NULL；签名 = sha*WithRSA + NULL；
//! - RSA-PSS：公钥同 RSA；签名 = RSASSA-PSS + PSS 参数
...
const SIG_ECDSA_SHA256: &[u8] = &[
    0x30, 0x0a, 0x06, 0x08, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x04, 0x03, 0x02,
];
/// ecdsa-with-SHA384（无参数）
const SIG_ECDSA_SHA384: &[u8] = &[
    0x30, 0x0a, 0x06, 0x08, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x04, 0x03, 0x03,
];
/// Ed25519（公钥/签名同形）
const ID_ED25519: &[u8] = &[0x30, 0x05, 0x06, 0x03, 0x2b, 0x65, 0x70];
/// rsaEncryption + NULL
const PK_RSA: &[u8] = &[
    0x30, 0x0b, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x01, 0x05, 0x00,
];
...
/// RSASSA-PSS（SHA-256 / MGF1-SHA256 / salt 32）
...
const SIG_RSA_PSS_SHA512: &[u8] = &[
    0x30, 0x34, 0xa0, 0x0f, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02,
    0x03, 0x05, 0x00, 0xa1, 0x1c, 0x30, 0x1a, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01,
    0x01, 0x08, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x03, 0x05,
