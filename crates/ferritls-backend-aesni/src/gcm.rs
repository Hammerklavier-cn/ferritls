//! AES-GCM 的 AES-NI + CLMUL kernel（全部安全代码）。
//!
...
impl NiGcm<11> {
    ...
    pub(crate) fn new(tok: &AesNi, key: &[u8; 16]) -> Self {
        Self::with_round_keys(tok, expand_128(tok, key))
    }
}
...
impl NiGcm<15> {
    ...
    pub(crate) fn new(tok: &AesNi, key: &[u8; 32]) -> Self {
        Self::with_round_keys(tok, expand_256(tok, key))
    }
}

impl<const N: usize> NiGcm<N> {
    fn with_round_keys(tok: &AesNi, round_keys: [[u8; 16]; N]) -> Self {
        let h_block = encrypt_block(tok, &round_keys, [0u8; 16]);
    ...
    fn seal_kernel(&self, nonce: &[u8; 12], aad: &[u8], buf: &mut [u8]) -> [u8; 16] {
        ...
        let tag_base = u128::from_be_bytes(encrypt_with(&self.tok, &keys, j0.to_be_bytes()));

        xor_keystream(&self.tok, &keys, inc32(j0), buf);

        let y = self.ghash(aad, buf);
    ...
    fn ghash(&self, aad: &[u8], ct: &[u8]) -> u128 {
        let mut y = 0u128;
        for_blocks(aad, |b| y = self.ghash_step(y, b));
        for_blocks(ct, |b| y = self.ghash_step(y, b));
        let mut len_block = [0u8; 16];
        len_block[..8].copy_from_slice(&((aad.len() as u64) * 8).to_be_bytes());
        len_block[8..].copy_from_slice(&((ct.len() as u64) * 8).to_be_bytes());
        self.ghash_step(y, &len_block)
    }

    /// 一步：y = (y ^ block) · H（原生约定）。
    #[inline]
    fn ghash_step(&self, y: u128, block: &[u8; 16]) -> u128 {
        let x = rev128(u128::from_be_bytes(*block));
        gfmul_native(&self.tok, y ^ x, self.h_rev)
    }
...
fn expand_128(tok: &AesNi, key: &[u8; 16]) -> [[u8; 16]; 11] {
...
fn expand_256(tok: &AesNi, key: &[u8; 32]) -> [[u8; 16]; 15] {
...
/// CTR 模式 keystream 异或覆盖 `data`（`ctr` 为起始计数器，软件约定）。
fn xor_keystream(tok: &AesNi, keys: &[__m128i], ctr_start: u128, data: &mut [u8]) {
    let mut ctr = ctr_start;
    for chunk in data.chunks_mut(16) {
        let ks = encrypt_with(tok, keys, ctr.to_be_bytes());
...
// —— GCM 计数器（与 core 软件实现相同的 RFC 5116 语义） ——
...
fn gfmul_native(tok: &AesNi, a: u128, b: u128) -> u128 {
    let (ah, al) = (((a >> 64) as u64), (a as u64));
    let (bh, bl) = (((b >> 64) as u64), (b as u64));
    let z0 = clmul64(tok, al, bl);
    let zc1 = clmul64(tok, ah, bl);
    let zc2 = clmul64(tok, al, bh);
    let z2 = clmul64(tok, ah, bh);
...
// —— 数据分块（GHASH 的 16 字节对齐，尾部零填充） ——

/// 按 16 字节遍历，末尾不足一块时零填充。
fn for_blocks(data: &[u8], mut f: impl FnMut(&[u8; 16])) {
...
mod tests {
    ...
    fn gfmul_native_properties() {
        ...
        assert_eq!(gfmul_native(&tok, a, 1), a, "a·1 = a");
        assert_eq!(gfmul_native(&tok, 1, a), a, "1·a = a");
        assert_eq!(gfmul_native(&tok, 0, a), 0);
        // 交换律。
        let b = 0xdeadbeefcafef00d1234567890abcdefu128;
        assert_eq!(gfmul_native(&tok, a, b), gfmul_native(&tok, b, a));
        // 约减抽查：X^65 的平方 = X^130 = X^128·X^2 ≡ X^9+X^4+X^3+X^2。
        let x65 = 1u128 << 65;
        let x130 = gfmul_native(&tok, x65, x65);
