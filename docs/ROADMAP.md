...
| M8 | intrinsics 后端（M8.1/8.2）、ML-KEM 混合与三参数集（M8.3/8.4）、QUIC（M8.5）、参数化收口轮（2026-09-17）已完成；余项 TLS 1.2 / aarch64 后端 + 候选 X448/P-521/Ed448 | 发布后 | **部分完成（2026-09）** |
...
- SHA-2 单流软件路径优化（P1.5 遗留热点，见上节"余留"；2026-09-17
  首轮尝试已实测回退——16 字滚动窗口消息调度在默认与 +avx2 更慢、
  仅 +avx512 回升且不消除回归，见 BENCHMARKS §5.5；再动手前需换
  思路，勿重复该改写）；
- 认证阶段 B/C 启动（见 docs/FIPS.md）。

### 参数化收口轮（完成 2026-09-17）

「算法支持多参数、实现写死单值」排查的落地轮：

- SHA-3 家族补全（SHA3-224/384，海绵核本就按 rate 参数化）+ 全家族
  KAT 进自检；
- HKDF 公开 API 宏化 + SHA-512 对（无 TLS 消费者，家族补全）；
- CCM 标签长度全参数化（M ∈ {4..16 偶数}，RFC 3610 §8 全部 24 个
  官方向量直跑）+ `TLS_AES_128_CCM_8_SHA256` 套件（仅默认模式，
  SP 800-52r2 批准面不含 8 字节标签）；
- 隐式省略转显式：sha2/ecdh/sign 模块头省略注记、下述候选余项条目。

### 候选余项（未排期；动手前先按本条目细化并补全出口条件）

- **X448**（RFC 7748）：`fp_field!` 新增 Fp448（8 limbs）→ 蒙哥马利
  阶梯复用 x25519 形态（钳制常量与位宽参数化）→ RFC 7748 §5.2/§6.1
  X448 向量 + Wycheproof x25519 Asn 组（如有 x448 组）→ kx.rs
  `NamedGroup::X448`（0x1E）+ 互操作。批准状态：非批准（SP 800-52r2
  只许经 ML-KEM 混合进入批准模式）。
- **P-521**（含 `ecdsa_secp521r1_sha512`）：`fp_field!` 新增 Fp521/
  Fp521Scalar（9 limbs，521 = 64×8 + 9 位需处理最高 limb 非全宽位掩码）
  → `sw_curve!` 实例 → RFC/Wycheproof ECDH+ECDSA 向量 → kx.rs/verify.rs/
  sign.rs 三处清单接线。批准状态：批准（SP 800-52r2 允许面含 P-521）。
- **Ed448**（RFC 8032）：Goldilocks 域 GF(2^448−2^224−1) 域算术 +
  SHAKE-256（`sha3::Shake256` 已有）→ RFC 8032 §7.3/§7.4 向量 →
  verify.rs SCHEME_MAP/sign.rs 接线。批准状态：非批准（FIPS 186-5
  不含 EdDSA）。

以上三项共同前置：`fp_field!` 对非整字位宽素数（521、448）的支持
论证（Montgomery 域常量推导、limb 布局、高位掩码不变式），动手前
先在分支上完成域算术原型并跑穷举/性质测试，再进入向量接线。
