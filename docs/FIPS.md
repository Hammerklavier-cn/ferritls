...
- 批准模式与硬件后端：`ops::install()` 在 `fips` feature 构建下直接
  拒绝，批准模式固定软件后端；是否将硬件后端纳入边界属阶段 C 的
  决策，不是默认权利。
...
| AES-128-CCM | SP 800-38C | 批准（算法级支持全 M ∈ {4..16 偶数}；TLS 批准套件面仅 M=16——CCM_8 仅默认模式） | 待 CAVP |
...
| SHA3-224/256/384/512、SHAKE-128/256 | FIPS 202 | 批准 | 待 CAVP（M8.3；2026-09 补齐 224/384 全家族） |
